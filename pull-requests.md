...
For guidance on when approval is required before merge, and when low-risk changes may proceed without waiting for peer approval, see the [Pull Request Approval Policy](./pull-request-approval-policy.md). Any references below to reviewers being "required" to check a changeset before it is merged should be read as subject to that policy (for example, Level 0 PRs and eligible Level 1 PRs after the configured waiting window may merge without prior peer approval).
